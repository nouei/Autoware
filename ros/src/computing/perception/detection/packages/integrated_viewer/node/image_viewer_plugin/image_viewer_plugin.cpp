...
void ImageViewerPlugin::on_image_topic_combo_box__activated(int index)
{
  ...
  image_sub_ = node_handle_.subscribe<sensor_msgs::Image>(selected_topic, 1, &ImageViewerPlugin::ImageCallback, this);
...
void ImageViewerPlugin::on_rect_topic_combo_box__activated(int index)
{
  ...
  if (topic_type.find(kRectDataTypeImageObjRanged) != std::string::npos)
  {
    ...
    rect_sub_ = node_handle_.subscribe<cv_tracker::image_obj_ranged>(selected_topic, 1,
                                                                     &ImageViewerPlugin::ImageObjRangedCallback, this);
  }
  else if (topic_type.find(kRectDataTypeImageObjTracked) != std::string::npos)
  {
    ...
    rect_sub_ = node_handle_.subscribe<cv_tracker::image_obj_tracked>(
        selected_topic, 1, &ImageViewerPlugin::ImageObjTrackedCallback, this);
  }
  else
  {
    image_obj_msg_ = NULL;
    ...
    rect_sub_ =
        node_handle_.subscribe<cv_tracker::image_obj>(selected_topic, 1, &ImageViewerPlugin::ImageObjCallback, this);
  }
...
void ImageViewerPlugin::on_point_topic_combo_box__activated(int index)
{
  ...
  // if selected topic is not blank or empty , start callback function
  point_sub_ =
      node_handle_.subscribe<points2image::PointsImage>(selected_topic, 1, &ImageViewerPlugin::PointCallback, this);
...
void ImageViewerPlugin::ShowImageOnUi(void)
{
  ...
  ui_.view_->setPixmap(view_on_ui.scaled(width, height, Qt::KeepAspectRatio, Qt::SmoothTransformation));
